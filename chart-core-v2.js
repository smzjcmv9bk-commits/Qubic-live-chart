...
function sync(v){v=valid(v)||active;active=v;for(const id of ['tf','qtf']){const e=$(id);if(e&&[...e.options].some(o=>o.value===v)&&e.value!==v)e.value=v}document.querySelectorAll('#quickTf [data-tf]').forEach(b=>b.classList.toggle('active',b.dataset.tf===v));const w=$('waterTf');if(w)w.textContent=v.toUpperCase();paintSource()}
function paintSource(){const src=chartSource||'ACTIVE';const t=$('chartToast');if(t)t.textContent=`${active.toUpperCase()} · ${src} OHLC · UK TIME`;const r=$('refSources');if(r&&chartSource)r.textContent=`Chart source · ${chartSource} · most active feed`}
...
window.fetch=async(input,init)=>{let raw=typeof input==='string'?input:(input?.url||'');if(!raw.includes('/api/mobile-candles'))return nf(input,init);const tf=active,cfg=TF[tf]||TF['15m'];try{const u=new URL('/api/active-candles',location.origin);u.searchParams.set('interval',cfg.src);u.searchParams.set('limit',String(chooseLimit(tf)));u.searchParams.set('requested',tf);if(chartSource)u.searchParams.set('prefer',chartSource);u.searchParams.set('t',String(Date.now()));const r=await nf(u.pathname+u.search,init);if(!r.ok)return r;const j=await r.clone().json();if(!j?.ok||!Array.isArray(j.candles))return r;if(j.source){chartSource=String(j.source).toUpperCase();paintSource()}const candles=aggregate(j.candles,tf);return new Response(JSON.stringify({...j,interval:tf,sourceInterval:cfg.src,requestedInterval:tf,aligned:true,timeZone:'Europe/London',candles}),{status:r.status,statusText:r.statusText,headers:{'Content-Type':'application/json','Cache-Control':'no-store'}})}catch{return nf(input,init)}};
...
window.__QubicChartCore={get timeframe(){return active},get source(){return chartSource},sync};
})();